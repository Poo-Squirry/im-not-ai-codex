...
# Humanize KR for Codex — 한글 AI 티 제거기 v2.0.0

[epoko77-ai/im-not-ai](https://github.com/epoko77-ai/im-not-ai)의 Codex 플러그인 포트입니다. 원본 공식 repo의 `v2.0.0` 기준으로 만들었고, 한국 번역학계 8유형 기반 taxonomy 보강과 post-editese metric layer까지 반영했습니다.

AI(ChatGPT · Claude · Gemini 등)가 쓴 한글 글을 **내용은 한 글자도 건드리지 않고** 문체 · 리듬 · 표현만 자연스러운 한국어로 되돌리는 Codex plugin/skill입니다.

번역투, 과도한 영어 인용, 기계적 병렬("첫째 · 둘째 · 셋째"), "결론적으로 / 시사하는 바가 크다" 같은 AI 특유 관용구, 피동태 남용, 문두 접속사 남발, 이모지·불릿 남용 등 **10대 카테고리 × 60+ 서브 패턴**을 심각도(S1/S2/S3)로 분류해 스팬 단위로 탐지한 뒤, 윤문합니다.
...
- Latest release: [`v2.0.0`](https://github.com/Squirbie/im-not-ai-codex/releases/tag/v2.0.0)
- Original release: [`epoko77-ai/im-not-ai v2.0.0`](https://github.com/epoko77-ai/im-not-ai/releases/tag/v2.0.0)
- Last upstream sync: `8071726` (v2.0.0 Korean translation scholarship + post-editese metrics)

## 버전 정책

이 Codex 포트는 원본 `epoko77-ai/im-not-ai`의 버전을 따라갑니다. 예를 들어 원본이 `v2.0.0`이면 이 plugin도 `v2.0.0`으로 배포합니다. 나중에 원본이 새 버전으로 올라가면, 원본 변경분을 반영한 뒤 같은 버전으로 새 release를 만듭니다.

즉 `im-not-ai-codex v2.0.0`은 "원본 `im-not-ai v2.0.0`의 taxonomy, metrics layer, fast/strict workflow를 Codex plugin 구조로 옮긴 배포판"이라는 뜻입니다. 전체 변경 내역은 [RELEASE_NOTES.md](./RELEASE_NOTES.md)를 참고하세요.
...
원본 `v2.0`의 핵심은 v1.6.1 fast path를 유지하면서 **한국 번역학계 8유형**과 Toral식 post-editese 축을 taxonomy/metrics에 붙인 것입니다. 연결어미 뒤 쉼표 같은 v1.6 신호에 더해 영어 대명사 직역, 관계절 좌향 수식, 이중 조사, 경어법 일관성 손실 같은 번역투 신호를 보조 점수로 계산합니다. fast mode 산출물은 계속 `final.md` 하나로 통합하고 메타데이터를 HTML 주석 블록으로 보존합니다.
...
입력 텍스트
    ↓
[metrics.py + metrics_v2.py] -- KatFish/LREAD + post-editese 보조 점수 계산
    ↓
...
| `korean-translation-scholar` | reference | 한국 번역학계 근거 검토 |
| `post-editese-metric-engineer` | reference | post-editese metric 설계 |
| `quick-rules-integrator` | reference | 본진 taxonomy를 fast 룰북으로 압축 |
| `taxonomy-gap-analyzer` | reference | 신규 번역투 gap 분석 |
| `translationese-research-distiller` | reference | 학술 근거 요약 |

## AI 티 분류 체계 (요약)

| ID | 대분류 | 대표 서브 패턴 |
|----|-------|---------------|
| A | 번역투 | "~를 통해", "~에 대해", "~에 있어서", 이중 피동 "~되어진다", "가지고 있다", 영어 대명사 직역, 관계절 좌향 수식, 이중 조사 |
| B | 영어 인용·용어 과다 | 과도한 괄호 병기, 번역 가능한 영어 그대로 |
| C | 구조적 AI 패턴 | 기계적 "첫째/둘째/셋째", 과도한 불릿·헤딩·이모지 |
| D | AI 특유 관용구 | "결론적으로", "시사하는 바가 크다", "주목할 만하다", "혁신적인" |
| E | 리듬 균일성 | 문장 길이 표준편차 낮음, 동일 종결어미 반복, 단문 일변도, 쉼표 분절 길이, 청자 경어법 일관성 손실 |
...
전체 60+ 서브 패턴과 처방은 plugin 내부 reference 문서에 들어 있습니다.

- [`quick-rules.md`](plugins/im-not-ai/skills/humanize-korean/references/quick-rules.md) — v2.0 fast path용 S1/S2 슬림 룰북
...
- [`metrics.py`](plugins/im-not-ai/skills/humanize-korean/references/metrics.py), [`baseline.json`](plugins/im-not-ai/skills/humanize-korean/references/baseline.json), [`metrics_v2.py`](plugins/im-not-ai/skills/humanize-korean/references/metrics_v2.py), [`baseline_v2.json`](plugins/im-not-ai/skills/humanize-korean/references/baseline_v2.json) — KatFish/LREAD + post-editese 정량 점수 레이어
- [`scholarship.md`](plugins/im-not-ai/skills/humanize-korean/references/scholarship.md) — v2.0 학술 근거 SSOT
...
공식 v2.0은 v1.6.1의 monolith fast path와 `final.md` 단일 산출물 정책을 유지하면서, 번역학 근거와 post-editese 정량 신호를 본진에 붙인 릴리즈입니다.

- **신규 본진 패턴** — A-16 영어 대명사 직역, A-18 관계절 좌향 수식, A-19 이중 조사 결합, E-7 청자 경어법 일관성 손실
- **기존 패턴 보강** — A-15 추상 주어, A-7 가지고 있다, F-4 과잉 접두·접미, E-2 동일 종결어미 처방 강화
- **post-editese metric layer** — `metrics_v2.py`와 `baseline_v2.json`으로 simplification, normalisation, interference 축을 계산
- **학술 근거 SSOT** — `scholarship.md`를 추가하고 taxonomy에 한국 번역학계 8유형 메타필드를 보존
- **A-17 hold** — 무정물·추상명사 `-들` 패턴은 근거와 metric은 보존하되, 본진 등재는 v2.1까지 보류
